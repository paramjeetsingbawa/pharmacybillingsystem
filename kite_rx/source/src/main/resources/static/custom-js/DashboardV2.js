...
$("#scheduleFilterTxt").bind('keypress', function (e) {
    if (e.which == 13) {
        getBpelInstances();
    }
});
...
$("#custNameForBillingTxt").bind('keyup', function (e) {
...
function calculateProfit() {
    try {
        ...
        var cgstInfo = document.getElementById("cgstTaxInfo").value;
        var sgstInfo = document.getElementById("sgstTaxInfo").value;
...
function addProductForBilling() {
    ...
    var productName = '';
    if (document.getElementById("bpelName")) {
        productName = document.getElementById("bpelName").innerHTML;
    } else if (document.getElementById("productNameFilterTxt")) {
        productName = document.getElementById("productNameFilterTxt").value;
    }
    ...
    try{
        var exppp = document.getElementById("expiryDateModel1").innerHTML;

        if(exppp === null || exppp.trim() === '' ){
            swal({title: "Blank Expiry Date For Product", 
                text: "Please add a valid expiry date for this product!"});
            return;
        } else {
                var sepChar = "/";
                if(exppp.includes("/")){
                    sepChar = "/";
                } else if(exppp.includes("-")){
                    sepChar = "-";
                } else if(exppp.includes(" ")){
                    sepChar = " ";
                } 

                var arr = exppp.split(sepChar);
                var monexp = arr[0];
                var yearExp = arr[1];
                
                var monthnamesfull = ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE', 'JULY', 'AUGUST', 'SEPTEMBER', 'OCTOBER', 'NOVEMBER', 'DECEMBER'];
                var monthnameshalf = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 
                    'SEP', 'OCT', 'NOV', 'DEC'];
                var monthnumbers = ['01', '02', '03', '04', '05', '06', '07', '08', '09', 'October', 'November', 'December'];;
                var date = new Date();

                var d1 = monthnamesfull[date.getMonth()] 
                         + sepChar + date.getFullYear();
                 var d2 = monthnameshalf[date.getMonth()] 
                         + sepChar + date.getFullYear();
                 var d3 = monthnumbers[date.getMonth()] 
                         + sepChar + date.getFullYear();
    ...
    if (isNaN(quantity) || parseFloat(quantity).toFixed(4) === '0.0000') {
        swal({title: "Validation Error", text: "Invalid input, please type a valid number(1..9)!"});
        return;
    }

    if (parseFloat(quantity).toFixed(4) === '0.0000'
            || parseFloat(quantity) <= 0.0 ||            
            isNaN(quantity) || isNaN(quantityInShop) || isNaN(amountPerUnit)) {
        if (parseFloat(quantity).toFixed(4) === '0.0000') {
    ...
    try {
        var batchNo = "";
        if (document.getElementById("batchModel1")) {
            batchNo = document.getElementById("batchModel1").innerHTML;
        }
        var alreadyAddedQuantuty = 0;
        var totalTableRef = document.getElementById("bpelInstSummayTabBody");
        //alert(totalTableRef.children.length);
        //alert(totalTableRef.children[1].children.length);
        for (var i1 = 0; i1 < totalTableRef.children.length; i1++) {
            //alert(totalTableRef.children[i1].children[2].innerHTML);
            if (batchNo.trim().length > 0) {
                if (productName === totalTableRef.children[i1].children[2].innerHTML) {
                    if (batchNo === totalTableRef.children[i1].children[4].innerHTML) {
                        var q1 = parseFloat(totalTableRef.children[i1].children[12].innerHTML);
                        quantityInShop = quantityInShop - q1;
                    }
                }
            } else {
                if (productName === totalTableRef.children[i1].children[2].innerHTML) {
            ...
            //alert("Now Quantity : " + quantityInShop);
        }
    } catch (Err) {
...
//    productDiscountArr.push(productDiscountPrice);
    ...
    try {
        ...
        document.getElementById("custBatchFilterTxt").value = "";
        document.getElementById("bpelID").value = "";
        document.getElementById("packing").value = "";
        document.getElementById("mrpTxt").value = "";
        document.getElementById("expiryTxt").value = "";
        document.getElementById("productNameFilterTxt").value = "";
    ...
    try {
        ...
        document.getElementById("totalTaxInfo").value = '';
        document.getElementById("cgstTaxInfo").value = '';
        document.getElementById("sgstTaxInfo").value = '';
...
var printProgressing = 0;

function print_specific_div_content()
{
    if(printProgressing === 1){
        return;
    }
        ...
        if (orderGenerated === 0) {
            ...
            if (paymentType === 'CREDIT') {
                if (creditAmount === '' || creditAmount === '0') {
                    ...
                    swal({title: "Validation Error", text: "Credit amount can not be 0, if payment type is credit!!"}, function () {
                        document.getElementById("creditAmount").focus();
                        document.getElementById("creditAmount").select();
                    });
            ...
            if (custName === null || custName === '' || custName.trim().length === 0) {
                swal({title: "Validation Error", text: "Customer name can not be empty!!"}, function () {
                    document.getElementById("custNameForBillingTxt").focus();
                    document.getElementById("custNameForBillingTxt").select();
                });
                return;
            }

            if (prescriberName === null || prescriberName === '' || prescriberName.trim().length === 0) {
                swal({title: "Validation Error", text: "Doctor name can not be empty!!"}, function () {
                    document.getElementById("prescriberNameForBillingTxt").focus();
                    document.getElementById("prescriberNameForBillingTxt").select();
                });
                return;
            }
            ...
            try {
                ...
                urlString += "&customerId=" + customerIDForBilling;
                ...
                if (document.getElementById("totalCreditAsOfNow")) {
                    urlString += "&totalCreditAsOfNow=" + document.getElementById("totalCreditAsOfNow").innerHTML;
                } else {
                    urlString += "&totalCreditAsOfNow=0";
                }

                urlString = encodeURI(urlString);
            } catch (err) {
                console.log("Error while geneting bill ..1 " + err);
                //alert(" " + err);
                swal({title: "UI Calculation Error!", text: "Error while generating bill.. " + err}, function () {
                    //document.getElementById("custNameForBillingTxt").focus();
                    //document.getElementById("custNameForBillingTxt").select();
                });
                return;
            ...
            $.ajax({
                type: "GET", url: urlString,
                success: function (data, text) {
                    printProgressing = 0;
                    swalParam({title: "Info", text:
                                "Total collective amount after additional discount applied : " + totalAmountFinal});
                    /* swal({title: "Info", text:
                     "Total collective amount after additional discount applied : " + totalAmountFinal}, function () {*/
                ...
                error: function (response, status, error) {
                    ...
                    console.log(status);
                    consol.elog(error);
                    swal({title: "Server Error!", text: "response.responseText : " + response.responseText + ", status : " + status + ", error : " + error}, function () {
                        //document.getElementById("custNameForBillingTxt").focus();
                        //document.getElementById("custNameForBillingTxt").select();
                    });
                }
...
function checkCreditAmount() {
    if (document.getElementById("paymentType").value === 'CREDIT') {
        document.getElementById('creditAmount').value = document.getElementById("amountTotalForFillWithDiscount").innerHTML;
        document.getElementById('creditAmount').disabled = false;
    } else if (document.getElementById("paymentType").value === 'PRE-PAID') {
        try {
            var i = parseFloat(document.getElementById("totalCreditAsOfNow").innerHTML);
            var j = parseFloat(document.getElementById("amountTotalForFillWithDiscount").innerHTML);

            if (i >= 0) {
                swal({title: "Payment Type Not Allowed", text: "PRE-PAID is only allowed if customer have pre-paid balance"});
                document.getElementById("paymentType").value = "CREDIT";
                document.getElementById('creditAmount').disabled = false;
                document.getElementById('creditAmount').value = document.getElementById("amountTotalForFillWithDiscount").innerHTML;
            } else if (i < j) {
                swal({title: "Information!", text: "pre-paid balance will become 0 and rest amount will be CREDIT for customer!"});
                document.getElementById('creditAmount').disabled = true;
                document.getElementById('creditAmount').value = j - i;
            }
        } catch (err) {
            document.getElementById("paymentType").value = "CREDIT";
        }
...
var generateBillInProgress = 0;
function generateBill() {
    
    if(generateBillInProgress === 1){
        return;
    }
    try{
        generateBillInProgress = 1;
        updateWindow = -4;
        currenNameListIndex = 0;
        currenNameListIndex1 = 0;
        currenNameListIndex2 = 0;
        ...
        for (var i1 = 0; i1 < totalTableRef.children[1].children.length; i1++) {
            ...
            cgstArr.push(totalTableRef.children[1].children[i1].children[9].innerHTML);
            sgstArr.push(totalTableRef.children[1].children[i1].children[10].innerHTML);
            igstArr.push('0.0');
...
function confirmedUpdate(updatePurchase) {
    ...
    document.getElementById("amountAddTxt").value = (parseFloat(mrpt) / parseFloat(packingt)).toFixed(4);

//    if (document.getElementById("cgstAddTxt").value !== "" && !allnumeric(document.getElementById("cgstAddTxt"))) {
//        //alert("");
//        swal({title: "Validation Error", text: "'CGST' should only be number(0..9 digits)!"}, function () {
//
//        });
...
//        //alert("");
//        swal({title: "Validation Error", text: "'SGST' should only be number(0..9 digits)!"}, function () {
//
//        });
...
function getBpelInstances(sortField, sortOrder) {
    ...
    var scheduleFilterTxt = document.getElementById("scheduleFilterTxt");
    if (scheduleFilterTxt) {
        if ($.trim(scheduleFilterTxt.value) != "") {
            urlString += "&schedule=" + scheduleFilterTxt.value;
            filterset = 1;
        }
        //batchNoFilterTxtValue.value = "";
    }
...
//            });
            ...
            $('#testInst').on('click', 'td', function () {
                ...
                if (document.getElementById(trid + "_cgst")) {
                    document.getElementById("cgstTaxInfo").value = document.getElementById(trid + "_cgst").innerHTML;
                    var c = document.getElementById(trid + "_cgst").innerHTML;
                    totalTax += parseFloat(c);
                }

                if (document.getElementById(trid + "_sgst")) {
                    document.getElementById("sgstTaxInfo").value = document.getElementById(trid + "_sgst").innerHTML;
                    var s = document.getElementById(trid + "_sgst").innerHTML;
                    totalTax += parseFloat(s);
                }

                //document.getElementById("totalTaxInfo").value = totalTax;

                if (isNaN(totalTax)) {
                    document.getElementById("totalTaxInfo").value = "0.00";
                } else {
                    document.getElementById("totalTaxInfo").value = totalTax;
                }
...
function getAllCustomerNames(evnt) {
    ...
    var i1 = document.getElementById("custNameForBillingTxt").value;
    if (i1 !== '' && i1.length > 0) {
    ...
    $.ajax({
        ...
        if (data) {
            $.each(summary, function (i, theItem) {
                var row;
                row = $("<tr id='" + i + "'></tr>");
                row.append($("<td id='" + theItem["ID"] + "' onclick='selectCustomer(this)'><a href='#' style='font-weight:700;color: white;' >"
                        + theItem["NAME"] + "</a>"));
...
//            document.getElementById("custNameList").children[i1].style.backgroundColor = "rgb(255, 51, 51)";
...
function selectCustomer(ref) {
    ...
    document.getElementById("custNameForBillingTxt").value = a;
    document.getElementById("custNameForBillingTxt").style.border = '';
    document.getElementById("custNameMobileNumberTxt").focus();

    document.getElementById("custNameMobileNumberTxt").style.border = '0.17em solid orange';
...
function fetchCreditReport(name) {
    var urlString = "/AvonMedicalService/rest/LadgerService/getCustomers?name=" + name + "&id=" + customerIDForBilling;
    ...
    $.ajax({
        ...
        if (data) {
            $.each(summary, function (i, theItem) {
                var pa1 = "Pending Credit : <span id='totalCreditAsOfNow'>" + theItem["TOTAL_CREDIT_ASOFNOW"] + "</span>";
                var pa2 = "Total Sale Amount : " + theItem["TOTAL_BILL_AMOUNT"];
                var pa3 = "Max Credit Allowed : " + theItem["MAX_CREDIT_AMOUNT"];
                document.getElementById("creditReportDiv").innerHTML = pa1 + ", " + pa2 + ",<u> " + pa3 + "</u> <a href='SaleReportV2.html?custNameFilter=" + name + "' target='_blank'>View Complete Report</a>";

                try {
                    var a1 = parseFloat(theItem["TOTAL_CREDIT_ASOFNOW"]);
                    if (a1 < 0) {
                        document.getElementById("paymentType").value = "PRE-PAID";
                        document.getElementById("creditAmount").disabled = true;
                        document.getElementById("creditAmount").disabled = "true";
                    }
                } catch (exception) {
