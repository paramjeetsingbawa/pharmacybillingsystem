...
function getCustomerInstances(sortField, sortOrder, flag) {
    if (flag !== 10) {
    ...
    var urlString = "/AvonMedicalService/rest/CoreService/getPurchaseDetail?maxrecord=50&pageNo=" + pageNo + "&pageSize=" + pageSize;
    var someFilterSet = false;

    var productName = document.getElementById("productName");
    if (productName) {
        if ($.trim(productName.value) != "") {
            urlString += "&productName=" + productName.value;
            someFilterSet = true;
        }
        //statusFilterTxtValue.value = "";
    }

    var distributerName = document.getElementById("distributerName");
    if (distributerName) {
        if ($.trim(distributerName.value) != "") {
            urlString += "&distributerName=" + distributerName.value;
            someFilterSet = true;
        }
        //bpelFilterTxtValue.value = "";
    }
    ...
    if (company) {
        if ($.trim(company.value) != "") {
            urlString += "&company=" + company.value;
            someFilterSet = true;
        }
        //bpelFilterTxtValue.value = "";
    }
    ...
    if (!someFilterSet) {
        ...
        document.getElementById("filterSpn").innerHTML = " (Default Current Month Purchases : " + today + ")";
    } else {
    ...
    $.ajax({
        ...
        if (data) {
            ...
            if (responseLength <= 0) {
                ...
                row.append($("<td id='" + rowid + "_totalAmountPaid" + "' style='font-weight:700;font-size:20px;color:green;'>").text("Total"));
            ...
            $.each(summary, function (i, theItem) {
                ...
                row.append($("<td id='" + rowid + "entryDate" + "'>").text(theItem["entryDate"]));
                row.append($("<td id='" + rowid + "productName" + "'>").text(theItem["productName"]));
                row.append($("<td id='" + rowid + "batch" + "'>").text(theItem["batch"]));
                row.append($("<td id='" + rowid + "quantityPurchased" + "'>").text(theItem["quantityPurchased"]));
                row.append($("<td id='" + rowid + "free" + "'>").text(theItem["free"]));
                row.append($("<td id='" + rowid + "amount" + "'>").text(theItem["amount"]));

                if (!isNaN(theItem["amount"])) {
                    totalPrice = totalPrice + parseFloat(theItem["amount"]);
                }

                row.append($("<td id='" + rowid + "mrp" + "'>").text(theItem["mrp"]));
                row.append($("<td id='" + rowid + "expiry" + "'>").text(theItem["expiry"]));
                row.append($("<td id='" + rowid + "distName" + "'>").text(theItem["distName"]));
                row.append($("<td id='" + rowid + "distNo" + "'>").text(theItem["distNo"]));
                row.append($("<td id='" + rowid + "company" + "'>").text(theItem["company"]));
                row.append($("<td id='" + rowid + "productDiscount" + "'>").text(theItem["productDiscount"]));
            ...
            $('#testInst').on('click', 'td', function () {
                ...
                document.getElementById(trid).style.backgroundColor = "green";
...
function getInvoice() {
    ...
    $.ajax({
        ...
        if (data) {
            ...
            row.append($("<td id='" + rowid + "_totalAmountPaid" + "'>").text(""));
            row.append($("<td id='" + rowid + "_totalAmountPaid" + "'>").text(""));
            row.append($("<td id='" + rowid + "_totalAmountPaid" + "' style='font-weight:500;'>").text("Total"));
