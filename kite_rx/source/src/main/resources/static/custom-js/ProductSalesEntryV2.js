...
function getAllProductBatches1(evnt) {
    ...
    $.ajax({
        ...
        if (count === 0) {
            document.getElementById("custBatchList1Body").innerHTML = "<tr><td colspan='14'><font style='color: white;font-size:34px;font-weight:700'>No product found for sale, Press 'Esc' or 'Ctrl+F' to go back</font></td></tr>'"
        }
...
function taxValueChanged() {
    var totalTaxInfo = document.getElementById("totalTaxInfo").value;
    if (totalTaxInfo === '' || totalTaxInfo === 'NaN') {

    } else {
        var floatTax = parseFloat(totalTaxInfo);
        document.getElementById("cgstTaxInfo").value = (floatTax / 2).toFixed(1);
        document.getElementById("sgstTaxInfo").value = (floatTax / 2).toFixed(1);
    }

    var totalSgstTaxPer = document.getElementById("sgstTaxInfo").value;
    var amtPerUnit = document.getElementById("amountPerUnit").value;

    if (totalSgstTaxPer > 0.00) {
        document.getElementById("sgstPerUnit").value = ((parseFloat(amtPerUnit) * parseFloat(totalSgstTaxPer)) / 100).toFixed(1);
    } else {
        document.getElementById("sgstPerUnit").value = 0.0;
    }

    var totalCgstTaxPer = document.getElementById("cgstTaxInfo").value;

    if (totalCgstTaxPer > 0.00) {
        document.getElementById("cgstPerUnit").value = ((parseFloat(amtPerUnit) * parseFloat(totalCgstTaxPer)) / 100).toFixed(1);
    } else {
        document.getElementById("cgstPerUnit").value = 0.0;
    }

    document.getElementById("taxAmountPerUnit").value = (parseFloat(amtPerUnit) - ((parseFloat(amtPerUnit) * parseFloat(totalCgstTaxPer)) / 100 + (parseFloat(amtPerUnit) * parseFloat(totalSgstTaxPer)) / 100)).toFixed(1);
...
function selectBatch1(ref) {
    ...
    document.getElementById("custBatchFilterTxt").value = document.getElementById("" + idForBilling + "_BATCH").innerHTML;
    document.getElementById("productIdTxt").value = idForBilling;
    document.getElementById("bpelID").innerHTML = idForBilling;
    document.getElementById("bpelName").innerHTML = document.getElementById("" + idForBilling).innerHTML;
    document.getElementById("amountPerUnit1").innerHTML = document.getElementById("" + idForBilling + "_PURCHASE_RATE").innerHTML;
    document.getElementById("packing").value = document.getElementById("" + idForBilling + "_PACKING").innerHTML;
    document.getElementById("mrpTxt").value = document.getElementById("" + idForBilling + "_MRP").innerHTML;
    document.getElementById("expiryTxt").value = document.getElementById("" + idForBilling + "_EXPIRE_DATE").innerHTML;
    document.getElementById("custBatchFilterTxt").value = document.getElementById("" + idForBilling + "_BATCH").innerHTML;
//    document.getElementById("addQuantityForBillingTxt").style.border = '0.2em solid orange';
//    document.getElementById("addQuantityForBillingTxt").focus();
    ...
    var sgstTaxPer = 0.00;
    try {
        if (document.getElementById(idForBilling + "_cgst")) {
            document.getElementById("cgstTaxInfo").value = document.getElementById(idForBilling + "_cgst").innerHTML;
            var c = document.getElementById(idForBilling + "_cgst").innerHTML;
            cgstTaxPer = parseFloat(c).toFixed(4);
            totalTax += parseFloat(c);
        }

        if (document.getElementById(idForBilling + "_sgst")) {
            document.getElementById("sgstTaxInfo").value = document.getElementById(idForBilling + "_sgst").innerHTML;
            var s = document.getElementById(idForBilling + "_sgst").innerHTML;
            sgstTaxPer = parseFloat(s).toFixed(4);
    ...
    var a = document.getElementById(trid).innerHTML;
    a = a.substring(a.indexOf('>') + 1);

    a = a.substr(0, a.indexOf('</a>'));

    //document.getElementById("bpelName").innerHTML = a;


    document.getElementById("batchModel1").innerHTML = document.getElementById(trid + "_BATCH").innerHTML;
    document.getElementById("expiryDateModel1").innerHTML = document.getElementById(trid + "_EXPIRE_DATE").innerHTML;
    ...
    if (!isNaN(packing)) {
        if (unitStr === 'TAB'
            ...
            var a1 = parseFloat(document.getElementById(trid + "_QUANTITY_IN_SHOP").innerHTML);
    ...
    if (unitStr === 'TAB'
        ...
        amtPerUnit = document.getElementById("amountPerUnit").value;
    ...
    if (document.getElementById(trid + "_purchaseRate")) {
        document.getElementById("amountPerUnit1").innerHTML = document.getElementById(trid + "_PURCHASE_RATE").innerHTML;
    }
    ...
    var totalTax = 0.00;
    var totalCgstTaxPer = 0.00;
    var totalSgstTaxPer = 0.00;
    if (document.getElementById(trid + "_cgst")) {
        ...
        totalCgstTaxPer = parseFloat(c).toFixed(4);

        if (totalCgstTaxPer > 0.00) {
            document.getElementById("cgstPerUnit").value = (parseFloat(amtPerUnit) - ((parseFloat(amtPerUnit) * parseFloat(totalCgstTaxPer)) / 100)).toFixed(1);
            ;
        } else {
            document.getElementById("cgstPerUnit").value = 0.0;
        }
    ...
    if (document.getElementById(trid + "_sgst")) {
        ...
        totalSgstTaxPer = parseFloat(s).toFixed(4);
        if (totalSgstTaxPer > 0.00) {
            document.getElementById("sgstPerUnit").value = (parseFloat(amtPerUnit) - ((parseFloat(amtPerUnit) * parseFloat(totalSgstTaxPer)) / 100)).toFixed(1);
        } else {
            document.getElementById("sgstPerUnit").value = 0.0;
        }
        totalTax += parseFloat(s);
    ...
    document.getElementById("taxAmountPerUnit").value = (parseFloat(amtPerUnit) - ((parseFloat(amtPerUnit) * totalTax) / 100)).toFixed(1);
    ...
    try {
        document.getElementById("distributernameInfo").innerHTML = document.getElementById(trid + "_DISTUBUTER_NAME").innerHTML;
    } catch (err) {
...
//                });
    ...
    if (totalTax > 0.00) {
        ...
        swal("Tax information not found for product entry. Please select here, it will update in master database also. ");
        document.getElementById("totalTaxInfo").focus();
        document.getElementById("totalTaxInfo").style.border = '0.27em solid orange';
        document.getElementById("totalTaxInfo").click();
        taxTotalFound = 1;
    }
    
    
    var expDate = document.getElementById(trid + "_EXPIRE_DATE").innerHTML;
    if(expDate === null || expDate.trim() === ""){
        if (confirm("Expiry date entry not found for this product entry! Do you want to continue? ") === true) {
            text = "You pressed OK!";
        } else {
            text = "You canceled!";
        }
