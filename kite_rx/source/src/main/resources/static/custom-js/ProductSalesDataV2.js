...
$("#custNameFilterTxt").bind('keyup', function (e) {
    ...
    try {
        ...
        if (!arrowPressed) {
            ...
            var v = document.getElementById("custNameFilterTxt").value;
            if (v === '') {
                return;
            }
...
function getCustomerInstances(isExported) {
    ...
    $('#myModal').modal('hide');
    document.getElementById("custNameFilterTxt").focus();
    ...
    $.ajax({
        ...
        if (data) {
            ...
            $.each(summary, function (i, theItem) {
                ...
                row.append($("<td id='" + rowid + "' onclick='selectBatch1(this)'><a href='#' style='font-weight:700;color: white;' >" + theItem["PRODUCT_NAME"] + "</a>"));
                ...
                row.append($("<td id='" + rowid + "_QUANTITY_IN_SHOP" + "'>").text(theItem["QUANTITY_IN_SHOP"]));
                ...
                row.append($("<td id='" + rowid + "_MARGIN" + "'>").text(theItem["MARGIN"]));
...
function getProductReport(flag) {

    document.getElementById("nameListDiv1").style.visibility = "hidden";
    document.getElementById("batchListDiv1").style.visibility = "hidden";
    document.getElementById("pageLoadingImg").style.display = "block";
    var urlString = "/AvonMedicalService/rest/CoreService/getBillingDetailForProduct?maxrecord=50";

    var custNameFilterTxt = document.getElementById("custNameFilterTxt");
    var someFilterSet = false;
    if (custNameFilterTxt) {
        if ($.trim(custNameFilterTxt.value) !== "") {
            urlString += "&productName=" + custNameFilterTxt.value;
            someFilterSet = true;
        }
    }
...
//    }
    ...
    document.getElementById("filterSpn").innerHTML = "<b> Name : " + document.getElementById("custNameFilterTxt").value + ", Batch : " + document.getElementById("custBatchFilterTxt").value + "</b>";
    ...
    urlString = urlString + "&pageNo=" + pageNo + "&pageSize=" + pageSize;
    ...
    $.ajax({
        ...
        if (data) {
            ...
            $.each(data, function (i, theItem) {
                ...
                row.append($("<td id='" + rowid + "_billNo" + "'><a target='_blank' href='Invoice.html?billNo=" + theItem["billSerialNumber"] + "'>" + theItem["billSerialNumber"] + "</a>"));
                ...
                var t5 = 0.00;
                try {
                    var t11 = theItem["totalAmount"];
                    if (!isNaN(parseFloat(t11))) {
                        t5 = parseFloat(t11).toFixed(2);
                    }
                } catch (err) {
                    t5 = 0.00;
                }
                row.append($("<td id='" + rowid + "_totalAmount" + "' style='text-align:right;'>").text(t5));
                totalAmountBill = totalAmountBill + parseFloat(t5);
                $("#tabReportBody").append(row);
            });
...
function selectBatch1(ref) {
    ...
    document.getElementById("custBatchFilterTxt").value = document.getElementById("" + customerIDForBilling + "_BATCH").innerHTML;
    document.getElementById("custBatchFilterTxt").style.border = '';
...
function getAllCustomerNames(evnt) {
    var urlString = "/AvonMedicalService/rest/CoreService/getCustomerNames?";
